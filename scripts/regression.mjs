...
check('trace page labels the terminal synthetic', trace.includes('SYNTHETIC EXAMPLE'));
check('trace page notes the sandbox is synthetic', trace.includes('Sandbox runs on synthetic data'));
check('trace page states no availability enumeration', !trace.includes('SPECIFIED') && !trace.includes('>LIVE<') && !/\bis live\b/.test(trace));
check('trace page keeps dispatch governed', trace.includes('Dispatch is governed') && trace.includes('governed Jobs plane'));
